...
export const saveAssetRetirement = async (req: Request, res: Response) => {
  const { PlacaActivo, DocumentoAprobado, Descripcion, DestinoFinal, Fotografia, NumeroBoleta, Usuario } = req.body;
  ...
  try {
    ...
    res.status(201).json({ message: "Asset retirement created successfully", data: assetRetirement });
  } catch (error: any) {
...
//     res.status(200).json({ message: "List of asset retirements successful", data: assetRetirements });
//   } catch (error: any) {
//     res.status(500).json({ error: error.message });
//   }
// };
...
//       res
//         .status(200)
//         .json({ message: "Update asset retirement successful", data: updatedAssetRetirement });
//     } else {
